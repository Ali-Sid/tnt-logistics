...
app.post('/item-list', async (req, res) => {
    const { itemCode, itemName, selectedType, selectedCategory, count } = req.body;
    // Validate that itemCode and itemName are not null or undefined
    if (!itemCode || !itemName) {
        return res.status(400).send({ message: 'itemCode and itemName are required' });
    }

    // Find the type_id based on the selectedType
    const typeQuery = 'SELECT type_id FROM types WHERE type_name = ?';
    try {
        const [typeRows] = await db.query(typeQuery, [selectedType]);
        if (typeRows.length === 0) {
            return res.status(400).send({ message: 'Selected type does not exist' });
        }
        const typeId = typeRows[0].type_id;

         // Find the category_id based on the selectedCategory
         const categoryQuery = 'SELECT category_id FROM category WHERE category_name = ?';
         const [categoryRows] = await db.query(categoryQuery, [selectedCategory]);
         if (categoryRows.length === 0) {
             return res.status(400).send({ message: 'Selected category does not exist' });
         }
         const categoryId = categoryRows[0].category_id;

        const query = 'INSERT INTO item_list (item_code, item_name, count, type, category, type_id, category_id ) VALUES (?, ?, ?, ?, ?, ?, ?)';
        await db.query(query, [itemCode, itemName, count, selectedType, selectedCategory, typeId, categoryId]);
...
app.put('/item-list/:id', async (req, res) => {
    ...
    const query = 'UPDATE item_list SET item_code = ?, item_name = ?, type = ?, category = ?, count = ?, WHERE item_id = ?';
    try {
        await db.query(query, [itemCode, itemName, selectedType, selectedCategory, count, id]);
...
//endpoint to add entries into the catalogue_list table
app.post('/category-list', async (req, res) => {
    const entries = req.body;
    try {
        for (const entry of entries) {
            const { itemCode, itemName, catalogueCode, timestamp, description, dateInput } = entry;
            const query = 'INSERT INTO catalogue_list (ItemCode, ItemName, CatalogueCode, event_timestamp, cat_dscp, a2cdate) VALUES (?, ?, ?, ?, ?, ?)';
            await db.query(query, [itemCode, itemName, catalogueCode, timestamp, description, dateInput]);
        }
...
app.get('/category-list/:itemCode', async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT * FROM catalogue_list WHERE ItemCode = ?', [itemCode]);
        if (rows.length === 0) {
...
app.get('/category-list', (req, res) => {
    const query = 'SELECT * FROM catalogue_list';
...
app.get('/category-list/', async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM catalogue_list');
        res.json(rows);
    } catch (error) {
...
app.put('/category-list/:id', async (req, res) => {
    const { id } = req.params;
    const { CatalogueCode, ItemCode, ItemName } = req.body;
    const query = 'UPDATE catalogue_list SET CatalogueCode = ?, ItemCode = ?, ItemName = ? WHERE id = ?';
    try {
        await db.query(query, [CatalogueCode, ItemCode, ItemName, id]);
        res.status(200).send({ message: 'Catalogue updated successfully' });
    } catch (error) {
...
app.get('/item-count/:itemCode', async (req, res) => {
    const { itemCode } = req.params;
    try {
        const query = 'SELECT COUNT(*) as count FROM catalogue_list WHERE item_code = ?';
